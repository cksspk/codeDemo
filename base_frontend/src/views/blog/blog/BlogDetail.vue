...
<script>
  ...
  import {uploadImgToAliOss} from "@/api/common"
  ...
  export default {
    ...
    created() {
      ...
      this.imagesUploadApi ='' + "/tool/qiNiu";
    ...
    methods: {
        ...
        if (query !== '') {
          ...
          setTimeout(() => {
            ...
            listBlogTagList(query).then(response => {
                console.log('查询博客tag，返回：',response)
                this.blogTagOptions = response.data.list;
                this.loading = false;
              }
            );
          }, 200);
      ...
      handleEditorImgAdd(pos, $file) {
        ...
        //设置oss中图片分类
        formdata.append('host', 'content');
        uploadImgToAliOss(formdata).then((resp) => {
          // debugger
